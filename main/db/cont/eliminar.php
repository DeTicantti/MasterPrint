...
if (isset($_GET['id'])) {
    ...
    if (empty($idComment)) {
        ...
        $sql = "DELETE FROM contador WHERE maquinaid = $idComment";
                ...
                if (isset($request)) {
                    if ($request) {
                        if ($con->affected_rows>0) {
                            echo "<div class='sucess'><i class='fas fa-check-circle'></i> Maquina Borrada Correctamente</div>";
                        }else{
                            $errors[] = "Esta maquina no existe";
                        }
