...
$sql = "SELECT * FROM impresoras WHERE imp_mar LIKE (:keyword) ORDER BY imp_id  ASC LIMIT 0, 1";
...
foreach ($lista as $milista) {
	...
	$pais_nombre = '<select>
    <option selected>'. str_replace($_POST['palabra'], '<b>'.$_POST['palabra'].'</b>', $milista['imp_mar']).'</option>
    <select>';
	// Aquì, agregaremos opciones
    echo '<li onclick="set_item(\''.str_replace("'", "\'", $milista['imp_mar']).'\')">'.$pais_nombre.'</li>
    ';
}
