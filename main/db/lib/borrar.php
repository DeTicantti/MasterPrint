...
if (isset($_GET['id'])) {
    ...
    if (empty($idComment)) {
        ...
        $sql = "DELETE FROM libros WHERE id = $idComment";
                ...
                if (isset($request)) {
                    if ($request) {
                        if ($con->affected_rows>0) {
                            echo "<div class='sucess'><i class='fas fa-check-circle'></i>Archivo Borrado Correctamente</div>";
                        }else{
                            $errors[] = "Este archivo no existe";
                        }
